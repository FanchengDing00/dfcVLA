...
from dfcvla import config as dfcvla_config
...
class TrainConfig:
    ...
    # vggt_configs
    vggt: dfcvla_config.VGGTConfig = dataclasses.field(default_factory=dfcvla_config.VGGTConfig)
...
_CONFIGS = [
    ...
    #Personal Tasks
    TrainConfig(
        name="pi05_vggt_libero",
        project_name="DFCVLA",
        exp_name="reproduce_test",
        model=pi0_config.Pi0Config(pi05=True, action_horizon=10, discrete_state_input=False),
        weight_loader=weight_loaders.CheckpointWeightLoader("gs://openpi-assets/checkpoints/pi05_base/params"),
        pytorch_weight_path="./checkpoints/pi05_base",
        
        vggt = dfcvla_config.VGGTConfig(
            weight_path="./checkpoints/vggt",  # Path to VGGT checkpoint.
        ),
         
        data=LeRobotLiberoDataConfig(
            repo_id="/vla/datasets/physical-intelligence--libero",  # Path to local dataset or a HF repo_id(auto download).
            base_config=DataConfig(prompt_from_task=True),
            extra_delta_transform=False,
            # Norm stats are loaded from (assets_dir)/asset_id/norm_stats.json.
            # If assets are not set, the defaut "assets_dir" is "asset_dirs" of the TrainConfig, and "asset_id" is "repo_id" of TrainConfig.data.
            assets=AssetsConfig(
                assets_dir="/vla/datasets/norm_stats/openpi/", 
